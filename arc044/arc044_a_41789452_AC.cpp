/*
 * Author: kAsA02
 * Submission URL: https://atcoder.jp/contests/arc044/submissions/41789452
 * Submitted at: 2023-05-28 03:30:21
...
bool prime_like(int n){
    if(n==1) return false; // 1は素数ではない,かつ合成数でもない
    ...
    if(ok) return true; // 素数
        ...
        if(tmp%2!=0 && tmp!=5 && sum%3!=0) return true; // 一桁目が5でなく、各桁の和が3の倍数でない時
